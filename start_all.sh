...
run_window() {
    ...
    tmux send-keys -t "$SESSION_NAME:$index" "cd \"$DEPLOY_DIR\" && source \"$VENV_PATH\" && exec python3 \"$script\"" C-m
}
...
echo "[3/4] 启动 万灵宗 (Xiaohao)..."
run_window 2 "Xiaohao" "cultivator_xiaohao.py"

echo "[4/4] 启动 云端监控台 (Dashboard)..."
run_window 3 "Dashboard" "dashboard_server.py"
