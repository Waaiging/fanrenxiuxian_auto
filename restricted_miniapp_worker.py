...
class RestrictedMiniAppWorker:
    ...
    async def _run_miniapp_heart_trial(self, identity: str) -> bool:
        """Never run the group-only heart-trial command from restricted Mini App mode."""
        self.log.info(
            "[%s] Restricted heart trial skipped: .共历心劫 is Telegram group-only",
            identity,
        )
        return True
        ...
        while self.actor.is_running:
            try:
                ...
                for identity in self.identities():
                    ...
                    # .共历心劫 requires a Telegram group reply anchor. Restricted
                    # accounts cannot safely execute it, so this worker never starts it.
            except asyncio.CancelledError:
                raise
            except Exception:
