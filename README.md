...
- 主号主魂与 Waaiging 的小世界状态、显灵和神迹布道走专用 Mini App 小世界入口；显灵只在存在祈愿且满足资源条件时执行。
- 实时监听机器人 `【小世界·天降浩劫】` 消息，按各账号自己的用户名匹配并安排“安抚信徒”。香火库存加待收仍不足时，按产速估算等待；够用后收割一次并安抚。神谕冷却按页面剩余时间安排；香火预计还需超过 3 小时加 30 秒时，布道可先执行并重算产速，否则保留共用冷却给安抚。
