...
- 推命闭关可勾选“服用合气丹”。每两次确认 `.闭关修炼` 成功后发送 `.服用 合气丹1`，成功后立即再发一次 `.闭关修炼`；回复明确表示丹药不足时自动取消勾选并向 `@Waaiging` 告警，后续继续普通推命闭关。
...
Mini App 主要配置位于 `miniapp_beast`，包括固定入口、问心塔、寻宝、天机试炼时间、天星宗游历、星宫灵圃和重试时间；受限账号补充配置位于 `restricted_miniapp`。Dashboard“自动化设置”保存在运行时文件 `automation_settings.json`（不提交 Git），包含青元子参战身份、慕兰参数、灵溪垂钓、天机试炼身份和天星宗闭关/天机值设置。配置文件迁移会自动补齐新增区段。
