...
Mini App 主要配置位于 `miniapp_beast`，包括固定入口、问心塔、寻宝、天星宗游历、星宫灵圃和重试时间；受限账号补充配置位于 `restricted_miniapp`。Dashboard“自动化设置”保存在运行时文件 `automation_settings.json`（不提交 Git），包含青元子参战身份、慕兰参数、灵溪垂钓参数和天星宗闭关/天机值设置。配置文件迁移会自动补齐新的 `tianxing` 区段。
