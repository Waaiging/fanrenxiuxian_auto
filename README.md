...
- 日常闭关循环执行 `.闭关修炼`，按实际回执冷却排期。每次执行前都按当前宗门判断：天星宗固定选择 `.定命 紫微`，确认当天命星为紫微后发送、确认 `.推命 闭关`，其他宗门直接闭关。已确认当天紫微时不重复定命；候选没有紫微或定命未确认时等待重试，不回退到其他命星。完整 worker 和受限 Mini App worker 共用该流程，定命或推命未确认时不会闭关。
...
- 每日北京时间 00:00 后发送一次 `.观命`，只保存当天返回的候选命星，不立即固定命星。已参与日常闭关的天星宗身份固定使用紫微，野外历练/探寻裂缝、炼制也不会改成其他命星；等待发送锁或 Mini App 认证期间切入日常模式，会拦截先前排队的其他定命指令。其他身份仍在动作前按优先级选择候选：闭关修炼优先 `紫微 → 贪狼`，野外历练/探寻裂缝优先 `贪狼 → 太阴`，炼制优先 `天府 → 太阴`。
