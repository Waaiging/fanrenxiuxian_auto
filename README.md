...
- 慢 `/charge-start` 回执会使服务端实际蓄力短于本地按住时间；在完美到达窗口与合法按住时长仍可兼顾时，会根据票据回执时间与正常网络延迟稍后释放。上报时长仍是实际按下到释放的间隔，成绩仍以服务端为准。诊断 v3 分别记录真正的唤醒延迟、蓄力前检查点保存耗时和票据回执时刻。
- 2026-09-11 13:40 场四账号均完成参战、验证及甲等 100 分结算，服务端完美次数依次为主号 0、副号 12、小号 13、Waaiging 16。主号沿用 731ms 慢校时样本而后续请求约 200ms，另有迟到窗口和慢蓄力丢完美。以上修复已覆盖对应的离线到达时刻、真实票龄及本地 HTTP 超时回归；修复后的实战次数仍需下一场验证。
