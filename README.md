...
- 主魂优先点击原题的答案按钮，以登录的 Telegram 用户响应该题，不需要先发 `.切换 主魂`。完整核对同一题的 A/B/C/D 按钮及答案，按钮移除或变化时停止；化身及没有按钮的普通题面仍通过身份管线回复 `.作答 A/B/C/D`。
- 在 300 秒有效期内作答，发送前复核题面、参与开关、暂停和指令守卫，并预留 10 秒余量。每题只提交一次，按钮响应不代表答对，以游戏判题为准；请求结果不明时不重试，也不转发群指令。重启仅恢复仍有效、尚未尝试发送的题目。
- 未知题和与群内判题冲突的答案存入 `xuangu_quiz_state.json`，面板列出题干、选项和回执提示；人工确认后用于后续题目。受限待机账号可通过原题按钮回答已启用主魂的已知题，其他题目只收集，不发送群指令或切换身份。
