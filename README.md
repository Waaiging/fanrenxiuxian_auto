...
- 推命闭关可勾选“服用合气丹”。每两次确认 `.闭关修炼` 成功后发送 `.服用 合气丹1`，成功后重新发送并确认 `.推命 闭关`，再执行一次 `.闭关修炼`；合气丹后的推命前置未确认时会阻止闭关。回复明确表示丹药不足时自动取消勾选并向 `@Waaiging` 告警，后续继续普通推命闭关。
- 切换到推命闭关时先发送 `.查看闭关`；若仍在深度闭关，则依次发送 `.强行出关`、`.服用 合气丹1`，确认完成后才进入普通推命闭关循环。切换到深度闭关时同样先发送 `.查看闭关`，确认未在深度闭关后直接发送 `.深度闭关`。每次 Dashboard 模式切换有独立轮次标识，避免进程重启后重复执行切换动作。
...
Mini App 主要配置位于 `miniapp_beast`，包括固定入口、问心塔、寻宝、天机试炼时间、天星宗游历、星宫灵圃和重试时间；受限账号补充配置位于 `restricted_miniapp`。Dashboard“自动化设置”保存在运行时文件 `automation_settings.json`（不提交 Git），包含青元子参战身份、慕兰参数、灵溪垂钓、野外深入历练身份、天机试炼身份和天星宗闭关/天机值设置。配置文件迁移会自动补齐新增区段。
