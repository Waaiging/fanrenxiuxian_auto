...
- 日常闭关循环执行 `.闭关修炼`，按实际回执冷却排期。每次执行前都按当前宗门判断：天星宗先选择 `.定命 紫微`，确认当前命星为紫微后发送、确认 `.推命 闭关`，其他宗门直接闭关。当前已确认紫微时不重复定命；其他任务切换命星后，下次闭关前重新定命紫微。候选没有紫微或定命未确认时只延后对应闭关，不回退到其他命星。完整 worker 和受限 Mini App worker 共用该流程，定命或推命未确认时不会闭关。
- 当天已确认的候选没有紫微时，属于正常等待，只记录一次提示并将该身份日常闭关排到次日 00:00 后重新观命；重启保留等待，游历、炼制与其他身份照常运行。当天候选更新、改宗或切换闭关模式后会重新判断；观命失败、空候选或定命未确认仍使用原有短期重试。
...
- 每日北京时间 00:00 后发送一次 `.观命`，只保存当天返回的候选命星，不立即固定命星。天星宗按即将执行的动作切换命星：日常闭关要求紫微，野外历练/探寻裂缝优先 `贪狼 → 太阴`，炼制优先 `天府 → 太阴`；日常模式不限制其他动作的定命。未参与日常闭关的身份仍沿用普通闭关的 `紫微 → 贪狼` 优先级。每次切换都确认回执，重新闭关前会从游历或炼制命星切回紫微。
