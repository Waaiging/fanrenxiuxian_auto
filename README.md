...
- `/window` 按请求发出时刻计算 360ms 轮询间隔，单次读取预算为 1 秒，超时后仍按原 `afterWindowId` 读取，避免一次慢查询占满下一击的准备时间。短读取预算耗尽不会开启或清除共享故障保护；已经开启的保护仍生效，恢复探测仍须收到真实回执。每个窗口汇总取数次数、等待次数、最长请求/轮询间隔、排队和回执处理耗时。
...
- 诊断 v7 为每个窗口保留最大轮询间隔的起止、前一次请求耗时、实际睡眠、迟唤醒与其余处理耗时，并记录超过 1 秒的间隔次数；只增加内存统计，不提高请求频率。这些分段用于定位后续长停顿，不能反推旧记录缺失的线程或内核证据。
