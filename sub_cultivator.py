...
STAR_GAZING_ROTATING_AVATARS = ["厚土"]
...
class SubCultivator(SurpriseRaidMixin, DuelMixin, CommonCommandMixin, ConcubineMixin, FishingMixin, YinluoMixin, SoulCurseMixin):
    ...
    def clear_retired_star_gazing_avatar_schedule(self):
        """Drop restored gazing/shift work assigned to an avatar no longer in rotation."""
        pending_avatar = str(self.state.get("star_gazing_claimed_avatar") or "").strip()
        if not pending_avatar or pending_avatar in STAR_GAZING_ROTATING_AVATARS:
            return False
        log.info(
            f"Star gazing: clearing retired avatar schedule for {pending_avatar}; "
            f"active rotation is {STAR_GAZING_ROTATING_AVATARS}."
        )
        self.clear_pending_star_shift()
        self.clear_star_gazing_round_claim()
    ...
    def stale_scheduler_due_items(self, overdue_seconds=SCHEDULER_STALE_DUE_SECONDS):
        """找出已到期很久却仍未推进的关键副号调度项。"""
        if self.state.get("is_paused") or self.identity_pause_seconds("主魂") > 0:
            return []
        specs = [
            ("next_yuanying_out_time", YUANYING_RETREAT_COMMAND, "主魂"),
            ("next_rift_search_time", ".探寻裂缝", "主魂"),
            ("next_ask_dao_time", ASK_DAO_COMMAND, "主魂"),
        ]
        if self.main_star_palace_enabled:
            specs.append(("next_treasure_touch_time", TREASURE_TOUCH_COMMAND, "主魂"))
        stale = []
        now = datetime.now()
    ...
    async def run_sub_health_watchdog_loop(self):
        ...
        while getattr(self, "is_running", True):
            try:
                ...
                if stale_fishing:
                    ...
                    log.critical(f"Sub watchdog: stale fishing active detected: {detail}; restarting process.")
                    self.save_state()
                ...
                if dead_tasks:
                    ...
                    log.critical(f"Sub watchdog: scheduler task stopped ({detail}); restarting process.")
                    self.save_state()
                    os.execv(sys.executable, [sys.executable, *sys.argv])
                ...
                log.error(f"Sub watchdog loop error: {exc}", exc_info=True)
    ...
    async def stop_for_rift_weakness(self, response, identity="主魂", msg=None):
        ...
        await send_text_alert(
            self,
            "副号探寻裂缝告警",
            f"探寻裂缝触发元婴虚弱期，副号身份【{identity}】已暂停；其他身份继续执行。\n\n"
    ...
    async def assist_avatar_formation_invite(self, formation_msg, initiator=""):
        ...
        candidates = [avatar for avatar in AVATAR_FORMATION_AVATARS if avatar != initiator and avatar in self.avatars]
        if not candidates:
    ...
    async def execute_avatar_formation(self, avatar):
        ...
        """
        if avatar not in AVATAR_FORMATION_AVATARS:
            log.info(f"Avatar [{avatar}] formation skipped: not a star-palace formation avatar.")
            return
        a_state = self.get_avatar_state(avatar)
        block_until = self.avatar_formation_block_until(avatar, a_state)
        if block_until:
        ...
        async def startup_sync():
            try:
                ...
                for avatar_name in self.avatars:
                    if avatar_name not in AVATAR_FORMATION_AVATARS:
                        continue
